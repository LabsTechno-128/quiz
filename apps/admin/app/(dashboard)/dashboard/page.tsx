...
const StatCard = ({ title, value, icon: Icon, trend, trendValue, color, loading }: {
  ...
  <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm hover:shadow-xl hover:shadow-indigo-500/5 transition-all duration-300 group">
    <div className="flex items-start justify-between">
      <div className={`p-4 rounded-2xl ${color} text-white shadow-lg shadow-indigo-200 transition-transform group-hover:scale-110 duration-300`}>
        <Icon className="h-6 w-6" />
      </div>
      <div className={`flex items-center gap-1 text-sm font-bold ${trend === "up" ? "text-emerald-500" : "text-rose-500"} bg-slate-50 px-2 py-1 rounded-lg`}>
    ...
    const fetchStats = async () => {
      try {
        ...
        setStats({
          ...
          quizzes: quizzesRes.data?.length || quizzesRes.data?.items?.length || 0,
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
        ...
        <div className="xl:col-span-2 bg-white p-8 rounded-[2rem] border border-slate-100 shadow-sm">
          <div className="flex items-center justify-between mb-8">
            <div>
              <h2 className="text-xl font-bold text-slate-900 tracking-tight">System Growth</h2>
            ...
            <div className="flex items-center gap-6">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-bold text-slate-500 uppercase">Quizzes</span>
              </div>
            </div>
          </div>
        ...
        <div className="bg-white p-8 rounded-[2rem] border border-slate-100 shadow-sm flex flex-col">
          <div className="mb-8">
            <h2 className="text-xl font-bold text-slate-900 tracking-tight">Content Distribution</h2>
            <p className="text-sm text-slate-400 font-medium">Resources by category</p>
          </div>
