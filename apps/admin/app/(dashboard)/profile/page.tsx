...
export default function ProfilePage() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const response = await privateRequest.patch("/user/profile/update", userData);
      if (response.data) {
    ...
    <div className="max-w-5xl mx-auto space-y-10 animate-in fade-in duration-500 pb-20">
      ...
      <div className="relative">
        ...
        <div className="absolute -bottom-16 left-12 flex items-end gap-6">
          ...
          <div className="pb-6">
            <h1 className="text-3xl font-extrabold text-slate-900 tracking-tight">{userData.name || "User"}</h1>
            <p className="text-slate-500 font-medium">System Administrator • Since 2026</p>
          </div>
        </div>
            ...
            {activeTab === "security" && (
              <div className="space-y-8 animate-in fade-in slide-in-from-right-4 duration-500">
                <div className="p-6 bg-amber-50 rounded-3xl border border-amber-100 flex items-start gap-4">
                  ...
                  <div>
                    <h4 className="text-sm font-extrabold text-amber-900">Two-Factor Authentication</h4>
                    <p className="text-xs text-amber-700 mt-1 font-medium leading-relaxed">Boost your account's safety by adding a second layer of security. We'll send a code to your phone every time you log in.</p>
                ...
                <div className="space-y-6">
                  <h4 className="text-lg font-bold text-slate-900 border-b border-slate-50 pb-4">Change Password</h4>
                  <div className="grid grid-cols-1 gap-6">
                    <div className="space-y-2">
                      <label className="text-sm font-bold text-slate-700 ml-1">Current Password</label>
                      <input type="password" placeholder="••••••••" className="w-full px-6 py-3.5 bg-slate-50 border border-slate-100 rounded-2xl outline-none focus:ring-2 focus:ring-indigo-500 transition-all" />
                    </div>
