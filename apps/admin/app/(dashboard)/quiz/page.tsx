      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {loading ? (
          ...
          quizzes.map((quiz: any) => (
            <div key={quiz.id} className="group bg-white p-6 rounded-3xl border border-slate-100 shadow-sm hover:shadow-xl hover:shadow-indigo-500/5 transition-all duration-300">
              <div className="flex items-start justify-between mb-6">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <h3 className="text-lg font-bold text-slate-900 group-hover:text-indigo-600 transition-colors">{quiz.title}</h3>
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-3 gap-4 mb-8">
                <div className="bg-slate-50/50 p-3 rounded-2xl text-center">
                  ...
                  <p className="text-xs font-bold text-slate-900">{quiz.questions?.length || 0}</p>
                  <p className="text-[10px] font-medium text-slate-500">Questions</p>
                </div>
                <div className="bg-slate-50/50 p-3 rounded-2xl text-center">
                  <Users className="h-4 w-4 text-violet-500 mx-auto mb-1.5" />
                  <p className="text-xs font-bold text-slate-900">{quiz.participantCount || 0}</p>
                  <p className="text-[10px] font-medium text-slate-500">Taken By</p>
                </div>
                <div className="bg-slate-50/50 p-3 rounded-2xl text-center">
                  <Clock className="h-4 w-4 text-pink-500 mx-auto mb-1.5" />
                  <p className="text-xs font-bold text-slate-900">{quiz.timeLimit ? `${quiz.timeLimit}m` : "No limit"}</p>
              ...
              <div className="flex items-center justify-between pt-6 border-t border-slate-50">
                <div className="flex items-center gap-2">
                  <div className={`h-2 w-2 rounded-full ${quiz.isActive !== false ? "bg-emerald-500" : "bg-amber-500"}`}></div>
                  <span className="text-xs font-bold text-slate-600">{quiz.isActive !== false ? "Active" : "Inactive"}</span>
                </div>
                <div className="flex items-center gap-2">
