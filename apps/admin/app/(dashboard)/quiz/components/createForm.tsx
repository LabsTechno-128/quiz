...
//   const id = params?.id as string;
  ...
  const fetchCategoryData = async () => {
    try {
      setLoading(true);
      const response = await privateRequest.get(`/quiz/${id}`);
  ...
  const onSubmit = async (values: any) => {
    try {
      ...
      if (isEditMode) {
        ...
        const res = await privateRequest.patch(`/quiz/${id}`, values);
  ...
  return (
   <div className="animate-in fade-in slide-in-from-bottom-4 duration-700">
      <PageHeader
        ...
        actionLink={{ href: "/quiz", label: "Back to List" }}
      />
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div className="space-y-6">
              ...
              <div className="flex items-center gap-4 p-4 bg-slate-50 rounded-[1.5rem] border border-slate-100 transition-all hover:bg-slate-100/50">
                <div
                  className={`w-12 h-6 rounded-full relative cursor-pointer transition-colors duration-300 ${status === "published" ? "bg-indigo-600" : "bg-slate-300"}`}
                  onClick={() => setValue("status", status === "published" ? "draft" : "published")}
                >
                  <div
                    className={`absolute top-1 left-1 bg-white w-4 h-4 rounded-full transition-transform duration-300 shadow-sm ${status === "published" ? "translate-x-6" : ""}`}
                  />
                </div>
                <div>
